...
- Permission keys follow `area.resource.action` (standard MVP set: `users.invite`, `users.manage`, `roles.manage`, `menu.publish`, `orders.cancel`, `payments.refund`, `analytics.read`, `audit.read`) - define new keys in `docs/data/domain-model.md` and the ticket, don't invent ad-hoc strings per feature.
