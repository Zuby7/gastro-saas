...
Ticket #58 (Epic 9 follow-up) adds `manual_sales_entries` -- a brand-new, structurally separate table for sales that happened outside this platform's own order/payment system (e.g. Lieferando, walk-in without the ordering system). Columns: `dish_id`, `quantity`, `sale_date`, optional `channel` (free text, e.g. "Lieferando"/"Vor Ort"), `entered_by_user_id`, `created_at`; tenant-scoped with RLS (`supabase/migrations/20260906090000_manual_sales_entries.sql`). Nothing in this table's read/write path ever touches `orders`/`order_items`/`payments`, and vice versa. Gated on a new, narrow `analytics.manual_sales.write` permission (Owner/Manager by default) -- deliberately not `menu.write` (this is sales data entry, not menu editing) nor `analytics.read` (read must never also authorize a write). `get_analytics_dashboard_summary()` and `get_dish_performance_stats()` both surface manual entries additively via dedicated `manualSales*`/`manualUnitsSold`/`manualEstimatedRevenueCents` fields (an ESTIMATE from the dish's current price, since a manual entry never went through real checkout pricing) -- never folded into the real-order-derived figures, and never used by the topseller/low-performer ranking (`classifyDishPerformance()`), so manual entries can never inflate a dish's ranking. Admin UI: a "Verkäufe nachtragen" section on the dish admin page (`apps/web/src/app/account/menu/dishes/[dishId]`), and a clearly-labeled, visually separate section/columns on the analytics dashboard and dish-performance pages.
