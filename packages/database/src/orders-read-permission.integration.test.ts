...
async function seedPaidPayment(admin: Client, tenantId: string, orderId: string, amountCents = 1500) {
...
describe.skipIf(!dbAvailable)("orders.read permission and staff dashboard (ticket #27, risk:tenant-isolation)", () => {
  ...
  it("a full list query for the dashboard board never returns another tenant's orders, even with matching statuses", async () => {
    const seed = await seedFixtureWithStaff();
    fixture = seed.fixture;
    await seedOrder(admin, fixture.tenantA.tenantId, "received", { customerName: "Tenant A Kunde 1" });
    await seedOrder(admin, fixture.tenantA.tenantId, "preparing", { customerName: "Tenant A Kunde 2" });
    await seedOrder(admin, fixture.tenantB.tenantId, "received", { customerName: "Tenant B Kunde 1" });
