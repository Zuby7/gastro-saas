...
- **Merge policy** (2026-08-01, after the auto-mode classifier correctly blocked a self-authored-self-merged PR): non-risk-labelled PRs merge automatically once the end-of-epic Opus batch review returns `APPROVED` — never before, and never self-merged by the same agent without that gate. Risk-labelled tickets (payments/auth/tenant-migration/`risk:*`) always require an explicit human merge decision, never automatic. See `.claude/skills/ship-ticket/SKILL.md`. Ticket #1's PR (#42) was merged before this policy was confirmed; it stands, but no further PR merges happen without it.
