...
| **restaurant** (folder: `restaurant`, doc originally called this `restaurant-profile`) | restaurant profile, opening hours, branding                                                                                                        | tenants                           |
| **menu**                                                                               | menus, categories, dishes, variants, option groups, options, allergens, additives, dietary labels, media, availability, publication/quality checks | tenants, restaurant               |
| **orders** (folder: `orders`, doc originally called this `ordering`)                   | carts, cart items, orders, order items, order state machine                                                                                        | menu, tenants                     |
| **payments**                                                                           | payment accounts, payments, refunds, webhook events                                                                                                | orders, tenants                   |
| **analytics**                                                                          | analytics events, daily aggregates, metric definitions                                                                                             | orders, payments, menu            |
| **sales-import** (ticket #58/#59)                                                      | manual sales entries, staged Excel/CSV import batches                                                                                              | analytics, menu, tenants          |
| **reviews**                                                                            | ratings, moderation                                                                                                                                | orders, tenants                   |
| **integrations**                                                                       | integration accounts, sync jobs, errors, mock provider                                                                                             | menu, orders, tenants             |
| **notifications**                                                                      | email sends, templates                                                                                                                             | identity, orders, payments        |
| **audit**                                                                              | audit log                                                                                                                                          | all (write-only append interface) |

Note (drift check, 2026-09-06): `identity`, `tenants`, `authorization`, `reviews`, and `notifications` above are documented as conceptual modules but do not (yet) have a corresponding `packages/domain/src/<module>/` folder — that logic currently lives under `apps/web/src/lib/*` instead. `packages/domain/src/` today only contains: `analytics`, `audit`, `cart`, `integrations`, `menu`, `orders`, `payments`, `restaurant`, `sales-import`. Per this doc's own collapse-to-single-app guidance below, this is an acceptable, already-happening drift, not a bug — flagged here rather than filed as a separate ADR, since it matches the "collapse packages/domain into apps/web/src/modules/*" escape hatch already described.
